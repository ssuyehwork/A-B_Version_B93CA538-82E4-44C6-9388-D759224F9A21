...
void FileSearchWidget::addSearchHistoryEntry(const QString& text) {
    ...
    QSettings settings("SearchTool_Standalone", "FileSearchFilenameHistory");
    QStringList history = settings.value("list").toStringList();
...
void FileSearchWidget::addExtHistoryEntry(const QString& text) {
    if (text.isEmpty()) return;
    QSettings settings("SearchTool_Standalone", "FileSearchExtHistory");
    QStringList history = settings.value("list").toStringList();
    history.removeAll(text);
    history.prepend(text);
    while (history.size() > 10) history.removeLast();
...
void FileSearchWidget::onFavoriteFile() {
    ...
    QSettings settings("SearchTool_Standalone", "FileSearchFileFavorites");
    ...
    // 如果左侧有选中项，过滤显示；否则显示全部
    QString filterPath;
    if (m_sidebar->currentItem()) {
        filterPath = m_sidebar->currentItem()->data(Qt::UserRole).toString();
    }
    refreshFileFavoritesList(filterPath);
    
    QToolTip::showText(QCursor::pos(), StringUtils::wrapToolTip("<b style='color:#2ecc71;'>✔ 已添加到文件收藏</b>"), this);
}
...
void FileSearchWidget::removeFileFavorite() {
    ...
    for (auto* item : selectedItems) {
        ...
        for (int i = 0; i < favs.size(); ++i) {
            if (favs[i].toMap()["path"].toString() == path) {
                favs.removeAt(i);
                break;
            }
        }
...
void FileSearchWidget::showFileFavoriteContextMenu(const QPoint& pos) {
    ...
    menu.addAction(IconHelper::getIcon("copy", "#2ECC71"), "复制路径", [path](){
        QApplication::clipboard()->setText(path);
    });

    menu.addSeparator();
    menu.addAction(IconHelper::getIcon("trash", "#E74C3C"), "取消收藏", this, &FileSearchWidget::removeFileFavorite);
...
void FileSearchWidget::saveFileFavorites() {
    // 已经在 onFavoriteFile 和 removeFileFavorite 中保存了
}

void FileSearchWidget::refreshFileFavoritesList(const QString& filterPath) {
    ...
    for (const auto& v : favs) {
        ...
        if (!filterPath.isEmpty()) {
            // 检查文件是否在 filterPath 目录下 (子目录也包含)
            if (!path.startsWith(filterPath)) continue;
        }

        auto* item = new QListWidgetItem(IconHelper::getIcon("file", "#4A90E2"), name);
        item->setData(Qt::UserRole, path);
        item->setToolTip(StringUtils::wrapToolTip(path));
...
FileSearchWindow::FileSearchWindow(QWidget* parent) 
    : FramelessDialog("查找文件 (独立版)", parent)
{
    setObjectName("FileSearchWindow_Standalone");
    resize(1000, 680);
